...
//         "rand"=>"desc"
    ...
    if($arParams["SORT_BY"] && $arParams["SORT_ORDER"]){
        $arSort = Array($arParams["SORT_BY"]=>$arParams["SORT_ORDER"]);
        //$arSort = Array("UF_ORIGINAL_NAME"=>"ASC");
    }
...
//        global $$arParams["FILTER_NAME"];
    ...
    //$arFilter = Array("SECTION_ID"=>$arResult["SECTION"]["ID"], "IBLOCK_ID"=>$arParams["IBLOCK_ID"]);

    if(is_array($filter)){
        $arFilter = array_merge($filter, $arFilter);
    }

    //print '<pre>' . print_r($arSort, true) . '</pre>';
    //EXECUTE
	$rsSections = CIBlockSection::GetList($arSort, $arFilter, $arParams["COUNT_ELEMENTS"], $arSelect);
    if($arParams["NAV_COUNT"]>0){
        $rsSections->NavStart($arParams["NAV_COUNT"]);
    }
