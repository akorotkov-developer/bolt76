...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
    ...
    if((!$bSectionFound) && (strlen($arParams["SECTION_CODE"]) > 0))
	{
        unset($arFilter["ID"]);
		...
		$arResult = $rsSection->GetNext();
		if($arResult) {
            if ($arParams["SECTION_ID"] && ($arParams["SECTION_ID"] != $arResult['ID'])) {
                header("HTTP/1.1 301 Moved Permanently");
                header("Location: ".$arResult['SECTION_PAGE_URL']);
                die();
            }
			$bSectionFound = true;
        }
	}
	elseif(!$bSectionFound)
	{
