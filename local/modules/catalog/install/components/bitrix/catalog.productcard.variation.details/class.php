	...
	implements Controllerable, Errorable
{
	...
	private function preparePictureFields(&$fields)
	{
	...
	private function parsePropertyFields(&$fields)
	{
		...
		foreach ($fields as $name => $value)
		{
			...
			{
				if (is_array($value) && isset($value['AMOUNT']) && isset($value['CURRENCY']) && \Bitrix\Main\Loader::includeModule('currency'))
				{
					$value = $value['AMOUNT'].\Bitrix\Currency\Integration\IblockMoneyProperty::SEPARATOR.$value['CURRENCY'];
				}
				$index = mb_substr($name, $prefixLength);
				$propertyFields[$index] = $value;
