...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		 ...
		 */
		refreshOrder: function(resultRefresh)
		{
			// TODO костыль для полчения картнок, выполним промисс для последовательного
			// исполнения кода
			console.log('Обновление заказа!!');

			console.log('this.result.GRID.ROWS!!!!!!!!!!!!!!!!');
			console.log(this.result.GRID.ROWS);
			var obGridRow = this.result.GRID.ROWS;
			var resultOb = false;

			var getProductsImages = new Promise((resolve, reject) => {
				const hPromiseContext = {resolve, reject};

				$.ajax({
					url: "/local/templates/stroyprofi/components/bitrix/sale.order.ajax/.default/ajax/getpictures.php",
					data: {'obGridRow': obGridRow},
				}).done(function(response) {
					resultOb = JSON.parse(response);
					console.log('resultOb:');
					console.log(resultOb);

					hPromiseContext.resolve('Успешное полчение картинок');
					hPromiseContext.reject('Ошибка получения картинок');
				});
			});

			var obContext = this;
			getProductsImages.then(
				result => {
					console.log('Выполнение второй части фнукции после выполнения промиса');
					if (resultOb !== false) {
						obContext.result.GRID.ROWS = resultOb;
					}

					obContext.refreshOrderAfterRefreshPictures(resultRefresh);
				},
			);
		},

		refreshOrderAfterRefreshPictures(result) {
			console.log('После обновления заказа result.GRID.ROWS');
			console.log(this.result.GRID.ROWS);
		...
		createBasketItemImg: function(data)
		{
			...
			console.log('data.PREVIEW_PICTURE_SRC:');
			console.log(data.PREVIEW_PICTURE_SRC)
