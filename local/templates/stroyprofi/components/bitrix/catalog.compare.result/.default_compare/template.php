...
<div class="bx_compare <? echo $templateData['TEMPLATE_CLASS']; ?>" id="bx_catalog_compare_block"><?
    ...
    <div id="tabs">
        <!-- Кнопки -->
        <ul class="tabs-nav">
            <?php foreach ($arResult['ALLOWED_SECTIONS'] as $sectionId => $section) {?>
                <li><a href="#<?= $sectionId?>" id="<?= $sectionId?>"><?= $section['NAME']?></a></li>
            <?php } ?>
        </ul>
        ...
        <div class="tabs-items">
            ...
            <?php foreach ($arResult['ALLOWED_SECTIONS'] as $sectionId => $section) { ?>
                <div class="tabs-item" id="<?= $sectionId?>">
                            ...
                            if (!empty($arResult["SHOW_FIELDS"])) {
                                foreach ($arResult["SHOW_FIELDS"] as $code => $arProp) {
                                    ...
                                    if ($showRow) {
                                        ...
                                        foreach ($arResult["ITEMS"] as $arElement) {
                                            if ($arElement['IBLOCK_SECTION_ID'] != $sectionId) {continue;}
                                            ...
                                            <td valign="top">
                                                ...
                                                switch ($code) {
                                                    case "NAME":
                                                        ...
                                                        <div class="b-compare-buttons">
                                                            <?php if ($arElement["CAN_BUY"]) { ?>
                                                                <noindex>
                                                                    <a class="btn btn-cart" href="<?= $arElement["BUY_URL"] ?>"
                                                                       rel="nofollow">
                                ...
                                foreach ($arResult["SHOW_OFFER_FIELDS"] as $code => $arProp) {
                                    ...
                                    if ($showRow) {
                                        ...
                                        <tr>
                                            ...
                                            <? foreach ($arResult["ITEMS"] as $arElement) {
                                                if ($arElement['IBLOCK_SECTION_ID'] != $sectionId) {
                                                    continue;
                                                }
                                                ?>
                                                <td><?
                                            ...
                                            if (count($rows) > 1) {
                                                foreach ($rows as $index => $rowData) {
                                                    ...
                                                    if ($rowData['QUANTITY_FROM'] == 0)
                                                        $rowTitle = GetMessage('CP_TPL_CCR_RANGE_TO', ['#TO#' => $rowData['QUANTITY_TO']]);
                                                    elseif ($rowData['QUANTITY_TO'] == 0)
                                                        $rowTitle = GetMessage('CP_TPL_CCR_RANGE_FROM', ['#FROM#' => $rowData['QUANTITY_FROM']]);
                                                    else
                                                        $rowTitle = GetMessage(
                                                            'CP_TPL_CCR_RANGE_FULL',
                                                            ['#FROM#' => $rowData['QUANTITY_FROM'], '#TO#' => $rowData['QUANTITY_TO']],
                                                        );
...
<script type="text/javascript">
    var CatalogCompareObj = new BX.Iblock.Catalog.CompareClass("bx_catalog_compare_block", '<?=CUtil::JSEscape($arResult['~COMPARE_URL_TEMPLATE']); ?>', window);
</script>