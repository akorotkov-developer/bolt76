...
class pricePrint
{
            ...
            foreach ($sectionElements as $elementKey => $element) {
                ...
                */
                ...
                if ($section['UF_TEMPLATE'] == 0) {
                    ...
                    $img = '';
                    $elementImage = CFile::ResizeImageGet($element['Foto'], array('width' => 100, 'height' => 1000), BX_RESIZE_IMAGE_PROPORTIONAL, true);
                    if ($elementImage) {
    ...
    private function setElementsXMlObjects()
    {
        ...
        foreach ($sections as $section) {
            $loadXML = simplexml_load_file($_SERVER['DOCUMENT_ROOT'] . '/tmp/section-' . $section['ID'] . '.xml');
    ...
    private function sortSectionsCats2Xml() {
        ...
        $xml = simplexml_load_file( $_SERVER['DOCUMENT_ROOT'] . '/import/preparedCatNew.xml');
        ...
        foreach ($arrXml['categories']['category'] as $key => $xmlItem) {
            if (!$arrSectionsWithKeys[$xmlItem['ID']['value']]['SORT_IN_PRICE']) {
                unset($arrXml['categories']['category'][$key]);
            } else {
        ...
        file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/tmp/cats_sort.xml', $result);
    }

    private function setSectionsXMLObject()
    {
        //Отсортируем категории файла cats2.xml
        $this->sortSectionsCats2Xml();

        $xml = simplexml_load_file($_SERVER['DOCUMENT_ROOT'] . "/tmp/cats_sort.xml");
            ...
            <items>
                ...
                foreach ($arItems as $item) {
                    ...
                    <item>
                        ...
                        <Naimenovanie><?=$item['NAME'];?></Naimenovanie>
                        ...
                        <?php }?>
                        <svertka><?=$item['PROPERTY_Svertka_VALUE'];?></svertka>
                    </item>
                    <?php
                    $countItems++;
                } ?>
    ...
    public function cacheDate(){
        $filename = $_SERVER['DOCUMENT_ROOT'] . '/tmp/cats2.xml';
        $result = date('d.m.Y');
...
<head>
    ...
    <?php require_once $_SERVER['DOCUMENT_ROOT'] . '/pricepdf/pdfview/pricePrintCss.php'; ?>
</head>
<body>
