...
class pricePrint
{
        ...
        if (!empty($sectionElements)) {
           ...
           /* usort($sectionElements, 'compare');*/
            ...
            foreach ($sectionElements as $elementKey => $element) {
                $getElementInfo = CIBlockElement::GetList(Array('PROPERTY_NAIMENOVANIE' => 'ASC'), array("PROPERTY_ROWID" => $element['ID'], "IBLOCK_ID" => 1), false, Array("nPageSize"=>10), array("ID", "NAME", "PREVIEW_PICTURE"));
    ...
    private function setSectionsXMLObject()
    {
        ...
        $xml = simplexml_load_file("tmp/cats_sort.xml");
    ...
    private function getItemsForXml() {
        ...
        $arFilter = [
            'IBLOCK_ID' => 1,
            'ID' => [159641, 159634, 159627, 159589],
        ];
        $dbRes = CIBlockElement::GetList(
            ['PROPERTY_NAIMENOVANIE' => 'ASC'],
            $arFilter,
...
<?php
$contentHtml = ob_get_contents();    // get the contents from the buffer
ob_end_clean();
...
$dompdf = new Dompdf();
$dompdf->loadHtml(htmlentities($contentHtml));
